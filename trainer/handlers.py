...
BTN_NEW = "🎯 Новый клиент"       # виден внутри тренажёра
BTN_STATS = "📊 Статистика"
BTN_EXIT = "🚪 Выйти"


def main_reply_kb():
    """Основная нижняя клавиатура (режим консультанта). Видна ВСЕГДА."""
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=BTN_TRAINER), KeyboardButton(text=BTN_STATS)],
        ],
...
def trainer_reply_kb():
    """Нижняя клавиатура внутри тренажёра."""
    return ReplyKeyboardMarkup(
        keyboard=[
...
def register_trainer(dp, bot, client, is_allowed=None):
    ...
    async def do_exit(message: Message, uid):
        was_in = SESSIONS.pop(uid, None) is not None
        text = (
        ...
        # Возвращаем ОСНОВНУЮ клавиатуру, а не убираем её — «Тренажёр» всегда под рукой
    ...
    async def start_in_session(message: Message):
        ...
        await message.answer(
            ...
            "Или жми «🎯 Тренажёр» внизу, чтобы снова потренироваться.",
